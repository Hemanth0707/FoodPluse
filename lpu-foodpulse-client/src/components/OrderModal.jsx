...
const OrderModal = ({ item, stall, onClose, onConfirm, walletPoints }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
      <div className="bg-[#12122a] border border-white/10 rounded-2xl max-w-md w-full overflow-hidden shadow-2xl animate-[slideUp_0.3s_ease-out]">
        
        {/* Header Image Area */}
        <div className="h-40 bg-gradient-to-br from-purple-900 to-pink-900 relative">
          <button 
            onClick={onClose}
          ...
          <div className="absolute -bottom-10 left-6">
            <div className="w-20 h-20 rounded-2xl bg-[#1e1e38] border-4 border-[#12122a] flex items-center justify-center shadow-lg text-3xl">
              🍔
            </div>
          </div>
        </div>
        ...
        <div className="p-6 pt-14">
          <div className="mb-6">
            <span className="text-purple-400 text-sm font-bold uppercase tracking-wider">{stall.name}</span>
          ...
          <div className="flex items-center gap-4 mb-8">
            <div className="flex-1 bg-white/5 border border-white/5 rounded-xl p-4 flex flex-col items-center justify-center">
              ...
              <span className="text-xl font-bold text-pink-400">{item.pointsCost} pts</span>
            </div>
            <div className="flex-1 bg-white/5 border border-white/5 rounded-xl p-4 flex flex-col items-center justify-center">
          ...
          <div className="border-t border-white/10 pt-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <span className={`font-bold ${walletPoints >= item.pointsCost ? 'text-green-400' : 'text-red-400'}`}>
                {walletPoints} pts
              </span>
            </div>
